...
public class CharacterController2D : MonoBehaviour {
	...
	void Update()
	{
		...
		if((isGrounded || canMidAirJump) && CrossPlatformInputManager.GetButtonDown("Jump")) // If grounded AND jump button pressed, then allow the player to jump
		{
			Jump();
		    if (!isGrounded) canMidAirJump = false;
		}
	    ...
	    rolling = CrossPlatformInputManager.GetButton("Fire2");
	    if (rolling)
	    {
