...
public class Enemy : MonoBehaviour
{
    ...
    // movement tracking
    [SerializeField] int _myWaypointIndex = 0; // used as index for My_Waypoints
    float _moveTime;
    float _vx = 0f;
    bool _moving = true;
    ...
    void Awake()
    {
        ...
        StartCoroutine(PeriodicallyJump());
    }
    ...
    IEnumerator PeriodicallyJump()
    {
        while (true)
        {
            yield return new WaitForSeconds(jumpPeriod + Random.Range(-0.5f, +0.5f));
            if (!isStunned)
                _rigidbody.AddForce(Vector2.up * jumpForce * Random.Range(0.8f, 1.2f), ForceMode2D.Force);
        }
    }
